...
PanelWindow {
    ...
    // Dismiss outright: drop the popup AND its history entry, so it does not
    // linger in the notification center. Used by the popup's X button.
    function dismissNotification(uid) {
        let notifRef = null;

        for (let i = 0; i < globalNotificationHistory.count; i++) {
            if (globalNotificationHistory.get(i).uid === uid) {
                notifRef = globalNotificationHistory.get(i).notif;
                globalNotificationHistory.remove(i);
                break;
            }
        }

        masterWindow.removePopup(uid);

        // Tell the sending app the user closed it (Notification has dismiss()/
        // expire() only -- there is no close()).
    ...
    NotificationServer {
        ...
        onNotification: (n) => {
            ...
            // The same uid goes into BOTH models so a popup can find (and drop)
            // its own history entry when dismissed.
            let notifData = {
                "uid": masterWindow._popupCounter,
