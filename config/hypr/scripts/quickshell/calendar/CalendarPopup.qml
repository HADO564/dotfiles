...
Item {
    ...
    // Slots for the orbit strip. View 0 is the rolling outlook, which crosses
    // midnight and so runs past today; the other views are that day's own slots.
    property var hourSlots: {
        if (!window.weatherData || !window.weatherData.forecast) return [];
        if (window.weatherView === 0 && window.weatherData.upcoming && window.weatherData.upcoming.length > 0)
            return window.weatherData.upcoming;
        let d = window.weatherData.forecast[window.weatherView];
        return (d && d.hourly) ? d.hourly : [];
    }

    property int activeHourIndex: {
        if (window.weatherView !== 0) return -1;
        let hrArr = window.hourSlots;
        if (!hrArr || hrArr.length === 0) return -1;

        // Slots carry an absolute dt, so nearest-in-time is exact even though the
        // rolling list repeats wall-clock times across the midnight boundary.
        // Caches written before dt existed fall back to comparing hours.
        let now = window.currentTime.getTime() / 1000;
        ...
        for (let i = 0; i < hrArr.length; i++) {
            ...
            let diff;
            if (dt > 0) {
                diff = Math.abs(dt - now);
            } else {
                let h = parseInt((hrArr[i].time || "00:00").split(":")[0]);
                let hd = Math.abs(h - ch);
                diff = Math.min(hd, 24 - hd) * 3600;
            }
            if (diff < minDiff) {
                minDiff = diff;
                bestIdx = i;
                ...
                Item {
                    ...
                    Repeater {
                        ...
                        delegate: Item {
                            ...
                            property real slotSpacing: Math.min(30, 300 / Math.max(1, mCount - 1))
                            property real targetAngleDeg: isToday ? (65 + (relIdx * slotSpacing)) : (index * (360 / Math.max(1, mCount)))
                            ...
                            Rectangle {
                                ...
                                ColumnLayout {
                                    ...
                                    // The rolling outlook runs past midnight, so the same
                                    // wall-clock time can appear twice; name the day when a
                                    // slot is not today's.
                                    Text {
                                        Layout.alignment: Qt.AlignHCenter
                                        visible: {
                                            let dt = modelData.dt || 0;
                                            if (dt <= 0) return false;
                                            let d = new Date(dt * 1000);
                                            return d.toDateString() !== window.currentTime.toDateString();
                                        }
                                        Layout.preferredHeight: visible ? implicitHeight : 0
                                        text: visible ? Qt.formatDateTime(new Date((modelData.dt || 0) * 1000), "ddd") : ""
                                        font.family: "JetBrains Mono"; font.weight: Font.Bold; font.pixelSize: Math.round(9 * window.sf)
                                        color: isHighlighted ? window.base : window.overlay0
                                    }
