...
# How many slots the rolling outlook carries. The free /2.5/forecast endpoint is
# fixed at 3-hour steps, so 12 slots == a 36h look-ahead that rolls past midnight.
UPCOMING_SLOTS="${OPENWEATHER_SLOTS:-12}"
...
# Pick the slot closest to right now, preferring the rolling `upcoming` list.
# Slots carry an absolute `dt`, so this is an exact comparison. Older caches
# written before `dt` existed fall back to clock-face distance: slot times are
# bare HH:MM and the list rolls past midnight, so a lexical "<= now" compare
# wrongly matches early-morning slots (at 14:35, "02:00" <= "14:35" is true).
current_slot() {
    local now_epoch now_min
    now_epoch=$(date +%s)
    now_min=$(( 10#$(date +%H) * 60 + 10#$(date +%M) ))
    jq -c --argjson now "$now_epoch" --argjson nowmin "$now_min" '
        (if ((.upcoming // []) | length) > 0 then .upcoming else (.forecast[0].hourly // []) end) as $h
        | if ($h | length) == 0 then null
          elif (($h[0].dt // 0) > 0) then
            $h | min_by(((.dt - $now) | fabs))
          else
            $h | min_by(
                (((.time | split(":") | (.[0] | tonumber) * 60 + (.[1] | tonumber)) - $nowmin) | fabs) as $d
                | if $d > 720 then 1440 - $d else $d end
            )
          end
    ' "$json_file"
}
...
write_dummy_data() {
    ...
    for i in {0..4}; do
        ...
        final_json="${final_json} {
            ...
            \"hex\": \"#cdd6f4\",
            \"desc\": \"No API Key\",
            \"hourly\": [{\"time\": \"00:00\", \"temp\": \"0.0\", \"icon\": \"\", \"hex\": \"#cdd6f4\", \"dt\": 0}]
        },"
    done
    final_json="${final_json%,}]"
    ...
    now_epoch=$(date +%s)
    dummy_up="["
    for i in $(seq 0 $((UPCOMING_SLOTS - 1))); do
        d_dt=$((now_epoch + i * 10800))
        d_time=$(date -d "@${d_dt}" "+%H:%M")
        dummy_up="${dummy_up} {\\"time\\": \\"${d_time}\\", \\"temp\\": \\"0.0\\", \\"icon\\": \\"\\", \\"hex\\": \\"#cdd6f4\\", \\"dt\\": ${d_dt}},"
    done
    dummy_up="${dummy_up%,}]"

    echo "{ \\"forecast\\": ${final_json}, \\"upcoming\\": ${dummy_up} }" > "${json_file}"
}
...
get_data() {
    ...
    if [ ! -z "$processed_forecast" ]; then
        ...
        for d in $dates; do
            ...
            # jq does the percentage itself; bc is not installed on this box and
            # its absence was silently leaving pop empty (UI showed "%" and NaN).
            f_pop_pct=$(echo "$day_data" | jq '(([.[].pop] | max) // 0) * 100 | floor')
            ...
            for i in $(seq 0 1 $count_slots); do
                ...
                hourly_json="${hourly_json} {\"time\": \"${s_time}\", \"temp\": \"${s_temp}\", \"icon\": \"${s_icon}\", \"hex\": \"${s_hex}\", \"dt\": ${s_dt}},"
            done
        ...
        # Rolling outlook for the bar/orbit strip. Slots are grouped into days by
        # the API's UTC date, which does not line up with the local day, so flatten
        # every day's slots and re-sort by absolute dt to get the true sequence.
        # Keep one step back (3h) so the slot currently in progress still counts.
        now_epoch=$(date +%s)
        upcoming_json=$(echo "{ \"forecast\": ${final_json} }" | jq -c --argjson now "$now_epoch" \
            '[.forecast[].hourly[]] | unique_by(.dt) | map(select(.dt >= ($now - 10800)))[0:'"${UPCOMING_SLOTS}"']')

        echo "{ \"forecast\": ${final_json}, \"upcoming\": ${upcoming_json} }" > "${json_file}"
    fi
}
...
# --- NEW HOURLY MODES FOR TOPBAR ---
    ...
    current_slot | jq -r '.icon // ""'

elif [[ "$1" == "--current-temp" ]]; then 
    t=$(current_slot | jq -r '.temp // "--"')
    echo "${t}°C"

elif [[ "$1" == "--current-hex" ]]; then
    current_slot | jq -r '.hex // "#cdd6f4"'
fi
