    ...
    delegate: Component {
        PanelWindow {
            ...
            Behavior on settingsSlideProgress {
                ...
                // Main.qml morphs the panel over 450ms opening / 350ms closing with
                // InOutCubic. Matching it makes the bar travel *with* the panel;
                // the old 80ms OutExpo teleported the boxes while the panel grew.
                NumberAnimation {
                    duration: barWindow.isSettingsOpen ? 450 : 350
                    easing.type: Easing.InOutCubic
                }
            }

            // Right edge of the settings panel, in bar-local coords. Read from the
            // same registry entry Main.qml uses, so changing the panel width there
            // moves the bar with it. The bar window is inset by its own left margin.
            property real settingsPanelWidth: {
                let t = Registry.getLayout("settings", 0, 0, barWindow.screen.width, barWindow.screen.height, scaler.uiScale);
                return t ? t.w : 0;
            }
            property real settingsPanelRight: settingsPanelWidth - barWindow.s(4)
            ...
            Item {
                ...
                Rectangle {
                    ...
                    // Sit just clear of the panel. Previously this packed backwards
                    // from rightContent, which parked the numbers ~280px away.
                    property real settingsX: barWindow.settingsPanelRight + barWindow.s(8)
                ...
                Rectangle {
                    ...
                    property real settingsX: workspacesBox.settingsX + workspacesBox.width + (workspacesBox.width > 0 ? barWindow.s(4) : 0)
                ...
                Rectangle {
                    ...
                    property real minCenterSettingsX: mediaBox.settingsX + mediaBox.width + (mediaBox.width > 0 ? barWindow.s(4) : 0)
                    // Stay centred unless the shifted left group would run into it,
                    // mirroring how defaultX behaves when settings is closed.
                    property real settingsX: Math.max(minCenterSettingsX, pureCenter)
                        ...
                        Row {
                            ...
                            Repeater {
                                ...
                                delegate: Image {
                                    ...
                                    // Themed popup instead of QsMenuAnchor, which
                                    // renders an unstyled native Qt widget menu.
                                    TrayMenu {
                                        id: menuAnchor
                                        anchorItem: trayIcon
                                        menuHandle: modelData.menu
                                        theme: mocha
                                        scaleFactor: barWindow.baseScale
                                        passthroughWindows: [barWindow]
                                    }
